...
class authController {
    ...
    async loginUser(req, res) {
        try {
            ...
            const countryCode =
                bodyCountryCode || (country && country.countryCode);
            ...
            const isRemembered =
                memorizedLogin === true || memorizedLogin === 'true';
    ...
    async forgotPasswordSendOTP(req, res) {
        ...
        if (!info) {
            ...
            return res
                .status(404)
                .json({ success: false, message: 'Info is null' });
        }
        ...
        if (info && regexVietnamPhoneNumber.test(info)) {
            try {
                ...
                if (!isProd) {
                    ...
                    console.log(
                        `EATSY FOOD - Reset OTP for ${formattedPhone}: ${otp}`,
                    );
                    console.log(
                        `==================================================\n`,
                    );
