...
const EMBEDDING_BASE_URL = process.env.EMBEDDING_BASE_URL || "http://127.0.0.1:11434/v1";
...
function isSemanticSearchEnabled() {
    return Boolean(EMBEDDING_BASE_URL && EMBEDDING_API_KEY && process.env.QDRANT_URL);
}
...
async function searchDishIdsBySemanticQuery(queryText, limit = 6) {
    ...
    return points
        ...
        .filter((point) => typeof point.dishId === "string" && point.dishId.length > 0);
}
