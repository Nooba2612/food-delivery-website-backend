...
const STOPWORDS = new Set([
    "va", "và", "la", "là", "cho", "toi", "tôi", "minh", "mình", "ban", "bạn",
    "co", "có", "khong", "không", "cai", "cái", "mon", "món", "loai", "loại",
    "tren", "trên", "duoi", "dưới", "nay", "này", "do", "đó", "kia", "nua", "nữa",
    "giup", "giúp", "them", "thêm", "voi", "với", "mot", "một", "nhung", "những",
]);

function stripDishCardTags(content) {
    return String(content || "").replace(/\[DISH_CARD:\s*\{.*?\}\]/g, " ").trim();
}
...
function pushUnique(list, value, limit) {
    ...
    const filtered = list.filter((item) => normalizePhrase(item) !== normalizedValue);
...
function extractPreferenceSignals(text) {
    ...
    const tastes = [
        "cay", "không cay", "ít cay", "ngọt", "ít ngọt", "mặn", "giòn", "béo",
        "healthy", "ít dầu", "nhiều đạm", "ăn kiêng",
    ].filter((signal) => normalized.includes(signal));

    const budgets = [
        "giá rẻ", "rẻ", "bình dân", "cao cấp", "dưới 50k", "dưới 100k", "tiết kiệm",
    ].filter((signal) => normalized.includes(signal));
...
function recordUserTurn(sessionId, message) {
    ...
    memory.recentUserIntents = pushUnique(memory.recentUserIntents, cleanMessage, 6);

    for (const keyword of signals.keywords) {
        memory.preferences.keywords = pushUnique(memory.preferences.keywords, keyword, MAX_KEYWORDS);
    }
    ...
    for (const budget of signals.budgets) {
        memory.preferences.budget = pushUnique(memory.preferences.budget, budget, 6);
    }
...
function recordAssistantTurn(sessionId, reply, cards = []) {
    ...
    memory.recentTurns = memory.recentTurns.slice(-MAX_STORED_TURNS);
    memory.lastAssistantCards = Array.isArray(cards) ? cards.slice(0, MAX_STORED_CARDS) : [];
...
function getLatestAssistantCardsFromHistory(chatHistory) {
    ...
    for (let index = history.length - 1; index >= 0; index -= 1) {
        ...
        if (entry?.role === "assistant" && Array.isArray(entry?.dishes) && entry.dishes.length > 0) {
            return entry.dishes;
        }
    }
...
function resolveOrdinalReference(message, cards = []) {
    ...
    const ordinalMatchers = [
        ...
        { pattern: /\b(món|cái|sp|sản phẩm|áo|quần)\s+cuối\b/, index: cards.length - 1 },
        { pattern: /\b(cái|món)\s+áp chót\b/, index: Math.max(cards.length - 2, 0) },
    ];
...
function resolveNamedReference(message, cards = []) {
    ...
    return cards.find((card) => normalized.includes(normalizePhrase(card?.name))) || null;
}
...
function resolveReferencedCard(sessionId, message, chatHistory) {
    ...
    const candidateCards = cardsFromHistory.length > 0
        ? cardsFromHistory
...
function messageNeedsContext(message) {
    ...
    const contextualSignals = [
        "món trên", "món đó", "món này", "loại đó", "loại này", "cái đó", "cái này",
        "cái kia", "ở trên", "ở dưới", "bên trên", "bên dưới", "thằng đó", "nó",
        "sp đó", "sản phẩm đó", "mẫu đó", "áo đó", "áo trên", "áo này", "quần đó",
        "món đầu", "món đầu tiên", "món cuối", "món thứ", "cái đầu", "cái cuối",
    ];
...
function buildMemorySummary(sessionId) {
    ...
    if (memory.preferences.taste.length > 0) {
        lines.push(`Khẩu vị quan tâm gần đây: ${memory.preferences.taste.join(", ")}`);
    }

    if (memory.preferences.budget.length > 0) {
        lines.push(`Ngân sách/mức giá quan tâm: ${memory.preferences.budget.join(", ")}`);
    }

    if (memory.preferences.keywords.length > 0) {
        lines.push(`Từ khóa người dùng hay nhắc: ${memory.preferences.keywords.slice(0, 8).join(", ")}`);
    }
...
function buildRetrievalContext({ sessionId, message, chatHistory = [] }) {
    ...
    const memory = ensureMemory(sessionId);
    const referencedCard = resolveReferencedCard(sessionId, currentMessage, chatHistory);
    const referencedCardContext = buildReferencedCardContext(referencedCard);
    const memorySummary = buildMemorySummary(sessionId);

    if (!messageNeedsContext(currentMessage) && !referencedCardContext && !memorySummary) {
        return {
